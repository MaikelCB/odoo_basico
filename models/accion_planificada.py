...
class accion_planificada(models.Model):
    ...
    def listado_facturas(self):
        ...
        self.env.user.tz = self.env['res.partner'].search([('id', '=', 3)])[
            0].tz  # como ten tz=UTC poñemoslle o tz do usuario=3 que é o administrador
        ...
        if facturas_ids:
            ...
            for rexistro in facturas_ids:
                ...
                listado = listado + "<br/>" + str(rexistro.name) + "-> " + str(
                    rexistro.partner_id.display_name) + "-> " + str(rexistro.amount_residual)
